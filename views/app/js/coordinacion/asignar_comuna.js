    ...
    success: function(json) {
      if (json.success == 1) {
        ...
        for (var i = 0; i < mes.length; i++) {
          $('#ejecutivos').append('<option class="opcion animated fadeIn" value="' + mes[i][0] + '">' + mes[i][0] + '</option>');
        }
      ...
      success: function(json) {
        if (json.success == 1) {
          ...
          for (var i = 0; i < usuariosNoAsignados.length; i++) {
            $('#usuariosNoAsignados').append('<tr><td><a data-placement="top" title="Asignar Supervisión" id="' + usuariosNoAsignados[i][0] + '" onclick="asignar_supervision('+ usuariosNoAsignados[i][0] +',' + user_id + ')" class="btn btn-success btn-md" ><i class="fa fa-user-plus" ></i></a></td><td class="opcion" value="' + usuariosNoAsignados[i][1] + '">' + usuariosNoAsignados[i][1] + '</td></tr>');
          }
          if (usuariosAsignados != undefined) {
            for (var i = 0; i < usuariosAsignados.length; i++) {
              $('#usuariosAsignados').append('<tr><td><a data-placement="top" title="Quitar Supervisión"  class="btn btn-danger btn-md btnDesasignar" id="' + usuariosAsignados[i][0] + '" onclick="quitar_supervision('+ usuariosAsignados[i][0] +')" ><i class="fa fa-user-times"></i></a></td><td class="opcion" value="' + usuariosAsignados[i][1] + '">' + usuariosAsignados[i][1] + '</td></tr> ');
            }
          }
